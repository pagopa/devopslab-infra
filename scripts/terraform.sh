...
function get_subscription_id() {
 local sub_name
 sub_name=$(grep "subscription=" "./env/$env/backend.ini" | cut -d'=' -f2)
 if [ -z "$sub_name" ]; then
   echo "Error: subscription name not found in backend.ini"
   exit 1
 fi
 ...
 if [ -z "$sub_id" ]; then
   echo "Error: Could not get subscription ID for $sub_name"
   exit 1
 fi
 echo "$sub_id"
}
...
function init_terraform() {
 if [ -n "$env" ]; then
   ARM_SUBSCRIPTION_ID=$(get_subscription_id) terraform init -reconfigure -backend-config="./env/$env/backend.tfvars"
 else
...
function other_actions() {
 if [ -n "$env" ] && [ -n "$action" ]; then
   ARM_SUBSCRIPTION_ID=$(get_subscription_id) terraform "$action" -var-file="./env/$env/terraform.tfvars" -compact-warnings $other
 else
...
function state_output_taint_actions() {
 if [ "$action" == "tflist" ]; then
   ...
   if command -v tflist &> /dev/null; then
     ARM_SUBSCRIPTION_ID=$(get_subscription_id) terraform state list | tflist
   else
     ARM_SUBSCRIPTION_ID=$(get_subscription_id) terraform state list | ./tflist
   fi
 else
   ARM_SUBSCRIPTION_ID=$(get_subscription_id) terraform $action $other
 fi
}
...
function tfsummary() {
 ...
 if [ -n "$(command -v tf-summarize)" ]; then
   ARM_SUBSCRIPTION_ID=$(get_subscription_id) tf-summarize -tree "${plan_file}"
 else

...
locals {
  apim_devopslab_webapp_python_alpha_api = {
    ...
    path                  = "devopslab-webapp-python-alpha"
    subscription_required = false
    service_url           = "http://${data.azurerm_public_ip.aks_pip.ip_address}/webapp-python-alpha"
    api_name    = "${var.env}-devopslab-webapp-python-alpha-api"
  }
}
...
module "apim_devopslab_webapp_python_alpha_api_v1" {
  ...
  content_value = templatefile("./api/devopslab/webapp-python/openapi_webapp_python.json", {
    host     = local.api_internal_domain # api.internal.*.userregistry.pagopa.it
    version  = "v1"
    basePath = local.apim_devopslab_webapp_python_alpha_api.path
  })

...
#   name                = "${local.project}-firewall-outbound-pip"
...
#   name                = "${local.project}-firewall-mng-outbound-pip"
#   location            = azurerm_resource_group.rg_vnet.location
#   resource_group_name = azurerm_resource_group.rg_vnet.name
#   allocation_method   = "Static"
#   sku                 = "Standard"
...
#   name                = lower("${each.key}-fw-net-rule-${local.project}")
...
#   name                = lower("${each.key}-fw-app-rule-${local.project}")
#   azure_firewall_name = azurerm_firewall.fw.name
#   resource_group_name = azurerm_resource_group.rg_vnet.name
#   priority            = 100 + (each.value.idx + 1)
#   action              = each.value.rule.action

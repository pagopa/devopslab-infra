...
module "zabbix_pg_flexible_snet" {
  ...
  name                                      = "${local.project}-pg-zabbix-snet"
...
module "zabbix_pgflex" {
  ...
  source = "git::https://github.com/pagopa/terraform-azurerm-v3.git//postgres_flexible_server?ref=v7.2.0"

  name                = "${local.project}-zabbix-pgflex"
